ddelap <- function(x, alpha, beta, lambda, log = FALSE){
  ...
  .Call('ddelap_f_wrap', x, alpha, beta, lambda, log, PACKAGE = 'DelF')
}
...
pdelap <- function(q, alpha, beta, lambda, lower.tail = TRUE, log.p = FALSE){
  ...
  if(!is.double(beta)) {storage.mode(beta) <- 'double'}
  if(!is.double(lambda)) {storage.mode(lambda) <- 'double'}
  .Call('pdelap_f_wrap', q, alpha, beta, lambda, lower.tail, log.p, PACKAGE = 'DelF')
}
...
qdelap <- function(p, alpha, beta, lambda, lower.tail = TRUE, log.p = FALSE, exact = TRUE, old = FALSE){
  ...
  if (exact) {
    QDLAP <- .Call('qdelap_f_wrap', p, alpha, beta, lambda, lower.tail, log.p, PACKAGE = 'DelF')
  } else {
...
MoMdelap <- function(x){
  if(!is.double(x)) {storage.mode(x) <- 'double'}
  MoMDLAP <- double(3)
  MoMDLAP <- .Call('momdelap_f_wrap', x, PACKAGE = 'DelF')
