...
expect_identical(qdelap(sfd, 1, 0.1, 0.05, lower.tail = FALSE), as.double(0:5))
...
expect_identical(qdelap(-1000, 1, 0.1, 0.05, lower.tail = FALSE, log.p = TRUE),
                 417)

# (T,T) deep lower-tail log target
q <- qdelap(-900, 4, 5, 5, log.p = TRUE)
expect_true(pdelap(q, 4, 5, 5, log.p = TRUE) >= -900)
expect_true(pdelap(q - 1, 4, 5, 5, log.p = TRUE) < -900)

# (T,T) round trip including the resolvable region
x <- 0:150
lp <- pdelap(x, 2, 0.2, 80, log.p = TRUE)
expect_identical(qdelap(lp, 2, 0.2, 80, log.p = TRUE)[lp < log(1 - tol)],
                 as.double(x)[lp < log(1 - tol)])
...
expect_true(all(is.finite(qq2)))
expect_true(all(diff(qq2) >= 0))
...
expect_warning(expect_identical(qdelap(-0.1, 2, 1, 1, lower.tail = FALSE), NaN))
expect_warning(expect_identical(qdelap(1.1, 2, 1, 1, lower.tail = FALSE), NaN))
...
expect_identical(qdelap(-Inf, 2, 1, 1, lower.tail = FALSE, log.p = TRUE), Inf)
expect_warning(expect_identical(qdelap(0.5, 2, 1, 1, lower.tail = FALSE,
                                       log.p = TRUE), NaN))
expect_identical(qdelap(double(0), 2, 1, 1, lower.tail = FALSE), double(0))
...
expect_identical(qdelap(lp, 2, 3, 0.1, log.p = TRUE), as.double(x))
...
expect_identical(qdelap(lsf, 20, 40, 0.1, lower.tail = FALSE, log.p = TRUE),
                 as.double(20:60))
