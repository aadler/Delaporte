...
test_that("Singleton qdelap functions", {
  ...
  expect_true(is.nan(qdelap(0, 1, 0, 2)))
...
test_that("Vector qdelap functions", {
  ...
  expect_identical(is.nan(pdelap(seq_len(2)/10, 0, 1, 2)), rep(TRUE, 2))
  expect_identical(is.nan(pdelap(seq_len(3)/10, c(0, 1, 2), c(1, 0, 2), c(1, 2, 0))), rep(TRUE, 3))
})
...
test_that("MoMdelap traps bad parameters", {
  ...
  expect_that(MoMdelap(TestData), throws_error("Method of moments not appropriate for this data; results include non-positive parameters."))
})